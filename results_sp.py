                    ...
                    for r in range(self.runs):    
                        ...
                        for ego_metr in self.ego_metrics:
                            ...
                            for k in self.ks:
                                ...
                                for error_metr in self.error_met: 
                                    if error_metr != 'mre_shortest_paths':
                                        ...
                                        error_1 = error_metrics.mre_shortest_paths( g1, ego_metrics_true[ego_metr], k=k)                   
                                        errors_list_1[ego_metr][error_metr][k].append(error_1)
                                        # utils.log_msg('g1 global %s %s = %s' % ( error_metr, ego_metr, error_1 ) )

                                        error_2 = error_metrics.mre_shortest_paths( g2, ego_metrics_true[ego_metr], k=k)                   
                                        errors_list_2[ego_metr][error_metr][k].append(error_2)
                                        # utils.log_msg('g2 global ds %s %s = %s' % ( error_metr, ego_metr, error_2 ) )

                                        # error_3 = error_metrics.mre_shortest_paths( g3, ego_metrics_true[ego_metr], k=k)                   
                                        # errors_list_3[ego_metr][error_metr][k].append(error_3)
                                        # # utils.log_msg('g3 local %s %s = %s' % ( error_metr, ego_metr, error_3 ) )
...
if __name__ == "__main__":
    ...
    error_met = [
                    'jaccard_shortest_paths', 
                    'mre_shortest_paths'
                ]

    ego_metrics = [ 
                    #   'strong_triangles2',
                       'strong_shortest_paths'  
                    ]

    runs = 3
    

    exp = ResultsDPWeightedNets(datasets_names, optins_methods, optins_perc, e, ks, error_met, ego_metrics, runs)
    exp.run()