                ...
                for optin_perc in self.optins_perc:
                    ...
                    for e in self.es:
                        ...
                        for r in range(self.runs):    
                            path_g1 = os.path.abspath(os.path.join(os.path.dirname( __file__ ), 'data', dataset, 'exp', 'graph_perturbed_%s_ins%s_e%s_r%s_global_ds.graphml' % ( optin_method, optin_perc, e, r )))
                            g1 = WGraph(path_g1)

                            path_g2 = os.path.abspath(os.path.join(os.path.dirname( __file__ ), 'data', dataset, 'exp', 'graph_perturbed_%s_ins%s_e%s_r%s_local_p3.graphml' % ( optin_method, optin_perc, e, r )))
                            g2 = WGraph(path_g2)
                    ...
                    for ego_metr in self.ego_metrics:
                        for error_metr in self.error_met: 
                    # utils.log_msg(' ======= %s =====' % dataset)
                    # utils.log_msg(' global ')
                    # utils.log_msg(errors_1)
                    # utils.log_msg(' local ')
                    # utils.log_msg(errors_2)
