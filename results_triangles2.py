            ...
            for optin_method in self.optins_methods: 
                for optin_perc in self.optins_perc:
                    ...
                    for ego_metric in self.ego_metrics:
                        ego_metrics_true[ego_metric] = egocentric_metrics.strong_triangles(g, self.threshold )
                        errors_1[ego_metric] = {}
                        errors_2[ego_metric] = {}
                        errors_3[ego_metric] = {}
                        # errors_4[ego_metric] = {}
                    ...
                    for r in range(self.runs):    
                        ...
                        for ego_metr in self.ego_metrics:
                            ego_metric_pred_1 = egocentric_metrics.strong_triangles(g1, self.threshold)
                            ego_metric_pred_2 = egocentric_metrics.strong_triangles(g2, self.threshold)
                            ego_metric_pred_3 = egocentric_metrics.strong_triangles(g3, self.threshold)
                            # ego_metric_pred_4 = egocentric_metrics.strong_triangles(g4, self.threshold)
                        
                            for k in self.ks:

                                for error_metr in self.error_met: 
                                    if error_metr != 'mre_triangles':
                                        ...
                                        error_1 = error_metrics.mre_triangles( g1, ego_metrics_true[ego_metr], k=k)                   
                                        errors_list_1[ego_metr][error_metr][k].append(error_1)
                                        # utils.log_msg('g1 global %s %s = %s' % ( error_metr, ego_metr, error_1 ) )

                                        error_2 = error_metrics.mre_triangles( g2, ego_metrics_true[ego_metr], k=k)                   
                                        errors_list_2[ego_metr][error_metr][k].append(error_2)
                                        # utils.log_msg('g2 global ds %s %s = %s' % ( error_metr, ego_metr, error_2 ) )

                                        error_3 = error_metrics.mre_triangles( g3, ego_metrics_true[ego_metr], k=k)                   
                                        errors_list_3[ego_metr][error_metr][k].append(error_3)
                                        # # utils.log_msg('g3 local %s %s = %s' % ( error_metr, ego_metr, error_3 ) )

                                        # error_4 = error_metrics.mre_triangles( g4, ego_metrics_true[ego_metr], k=k)                   
                                        # errors_list_4[ego_metr][error_metr][k].append(error_4)
                    ...
                    for ego_metr in self.ego_metrics:
                        for error_metr in self.error_met: 
                            ...
                            ylabel = "Jaccard Distance" if error_metr=="jaccard_triangles" else "MRE"
