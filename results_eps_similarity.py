                ...
                for optin_perc in self.optins_perc:
                    ...
                    for e in self.es:
                        ...
                        for r in range(self.runs):    
                            ...
                            for ego_metr in self.ego_metrics:
                                ...
                                for error_metr in self.error_met: 
                                    error_1 = egocentric_metrics.similar(g, g1)               
                    ...
                    for ego_metr in self.ego_metrics:
                        for error_metr in self.error_met: 
                            ...
                            path_result = "./data/%s/results/result_%s_%s_%s.png" % ( dataset, optin_method, optin_perc, ego_metr) 
                            graphics.line_plot2(np.array(self.es), np.array(y), xlabel='$\epsilon$', ylabel= 'similarity', ylog=False, line_legends=legends, path=path_result)                                
                            path_result2 = "./data/%s/results/logscale_result_%s_%s_%s_%s.png" % ( dataset, optin_method, optin_perc, ego_metr, error_metr) 
                            graphics.line_plot2(np.array(self.es), np.array(y), xlabel='$\epsilon$', ylabel= 'similarity', ylog=True, line_legends=legends, path=path_result2)                                
