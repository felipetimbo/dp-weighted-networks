...
class preprocessing():
    ...
    def run(self):
        for dataset in self.datasets_names:
            url = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'data', dataset, 'enron.graphml' ))
            url_output = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'data', dataset ))
            # edges = genfromtxt('%s/%s.csv' % (url, dataset), delimiter=',' )
            g_enron = gt.load_graph(url)
            
            g = gt.Graph(directed=False) 
            ew = g.new_edge_property('int') # edge weight
            g.edge_properties['ew'] = ew 

            edges = g_enron.get_edges([g_enron.ep.we])
            ...
            for optin_method in self.optins_method: 
                for optin_perc in self.optins_perc:
                    if optin_method == 'random':
                        self.optins_based_on_random_selection(g, optin_perc, dataset, url_output)
                    elif optin_method == 'affinity':
                        self.optins_based_on_affinity_model(g, optin_perc, dataset, url_output)
